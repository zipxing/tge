...
export class ElsGrid {
    ...
    setBmp(bmp: number[][]) {
        ...
        for (let i=0; i<constant.ZONG; i++) {
            for (let j=0; j<constant.HENG; j++) {
                mc.grid[i*constant.GRIDW + 2+j] =
                    (i>=(constant.ZONG-10)) ?
                    (100 + bmp[i - (constant.ZONG-10)][j]) :
                    0;
                if(mc.grid[i*constant.GRIDW + 2+j]==100) {
                    mc.grid[i*constant.GRIDW + 2+j]=0;
                }
            }
        }
    }
    ...
    reset() {
        ...
        for (let i=0; i<constant.ZONG+2; i++) 
            for (let j=0; j<constant.HENG+4; j++)
                mc.grid[i*constant.GRIDW + j]=200;
