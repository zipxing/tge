namespace Tetris {
    ...
    export class WebGlRender extends tge.Render {
        ...
        drawBlk(idx:number, type:BlockType, col:number, row:number, color: number, fullrow_stage: number = 0) {
            let mm = this._mesh2![idx*HENG*ZONG+row*HENG+col];
            let rr = <tge3d.MeshRenderer>mm.components[tge3d.SystemComponents.Renderer];
            row = ZONG-row;
            col = HENG-col;
            ...
            switch(type) {
                ...
                case BlockType.FULLROW: //满行闪烁
                    ...
                    mm.localRotation.setFromEulerAngles(new tge3d.Vector3(3-fullrow_stage*5, 13-fullrow_stage*5, 0));
