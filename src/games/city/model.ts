...
export class Model extends tge.Model {
        ...
        for(let i=0; i<HEIGHT; i++) {
            ...
            for(let j=0; j<WIDTH; j++)
                this.grid[i][j]={
                    id: i*WIDTH+j,
                    fromid: -1, toid: -1,
                    color: tge.rand() % (COLOR_COUNT-1) + 1,
                    level: tge.rand()%2 + 1
                };
        }
        this.merges = {objCell: this.grid[0][0], mergeCells:[]};
    ...
    searchUnit() {
        ...
        this.units = {};
        this.ready2TUnits = [];
        this.searchUnitByXRange(0, WIDTH);
    }
    ...
    drop() {
        ...
        for(let x=0; x<WIDTH; x++) {
            ...
            for(let y=0; y<HEIGHT; y++) {
                ...
                let dropcnt = 0;
                for(let n=y+1; n<HEIGHT; n++) {
                    let cc = this.grid[n][x];
                    if(cc.color==-1) dropcnt++;
                }
                c.fromid = c.id;
                c.toid =   c.id + dropcnt*WIDTH;
                blocks.push(c);
            }
            //set new block(reuse hole)...
            for(let i=0; i<holes.length; i++) {
                let h = holes[i];
                h.color = tge.rand()%COLOR_COUNT + 1;
                h.level = tge.rand()%2 + 1;
                h.fromid = (i-holes.length)*WIDTH + x;
                h.toid = i*WIDTH + x;
                h.id = h.toid;
                this.copyCell(h, tmpcs[i]);
            }
            for(let i=0; i<blocks.length; i++) {
                blocks[i].id = (i+holes.length)*WIDTH + x;
