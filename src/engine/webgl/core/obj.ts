    ...
    _toMesh(){
        ...
        if(this._normals.length > 0){
            if(this._normals.length !== this._vertices.length){
                log.warning("obj file normals count not match vertices count");
            }
        ...
        if(this._texcoords.length > 0){
            if(this._texcoords.length !== this._vertices.length){
                log.warning("obj file texcoords count not match vertices count");
            }
        ...
        for(let i=0; i<this._faces.length; i++){
            ...
            for(let j=0; j<face.vIndices.length; j++){
                ...
                if(face.nIndices.length > 0){
                    ...
                    if(nIdx !== vIdx){
                        //log.warning('obj file nIdx not match vIdx');
                    }
                }
            }
        }
        ...
        if(tangents.length===0 && this.calcTangent){
            if(uvs.length==0){
                log.error("Need uv coordinates to compute mesh tangents");
            } else {
        ...
        log.info('vertex count '+this._vertices.length);
        log.info('triangle count '+triangels.length/3);
