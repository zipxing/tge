...
export class VertexBuffer{
    ...
    constructor(vf: VertexFormat){
        ...
        for(let i=0; i<attribNum; ++i){
            ...
            if(size==null){
                log.error('VertexBuffer: bad semantic');
            } else {
    ...
    private _compile(){
        ...
        if(positionInfo == null){
            log.error('VertexBuffer: no attrib position');
            return;
        }
        if(positionInfo.data == null || positionInfo.data.length===0){
            log.error('VertexBuffer: position data is empty');
            return;
        }
        ...
        for(let i=0; i<this._vertex_count; ++i){
            for(let semantic of this._vertex_format.attribs){
                ...
                if(info==null || info.data==null){
                    log.error('VertexBuffer: bad semantic '+semantic);
                    continue;
                }
                for(let k=0; k<info.size; ++k){
                    let value = info.data[ i * info.size + k ];
                    if(value===undefined){
                        log.error('VertexBuffer: missing value for '+semantic);
                    }
